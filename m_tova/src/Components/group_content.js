...
const GroupContent = props => {
    ...
    // const getDetailStudent =(e)=>{
    //     console.log(e.target.id);
    //     storage.ref().child(`${course_name}/class${student_details.class_number}/${student_details.student_name}`).listAll().then(list => {
    //         console.log(`${course_name}/class${student_details.class_number}/${student_details.student_name}`);
    //         //console.log()
    //     })
                ...
                list = querySnapshot.docs.map(item => {
                    return (
                        <div className="shared-content-card"  key={item.data().student_name} >
                            ...
                            <ul>שיעור {item.data().class_number}</ul>
